...
class MRELBP():
    ...
    def MRELBP(self, image):
        ...
        write_to_filecheck("check_median/median_3x3_py.txt", m_3x3)
        write_to_filecheck("check_median/median_5x5_py.txt", m_5x5)
        write_to_filecheck("check_median/median_7x7_py.txt", m_7x7)
        ci_r2 = self.mrelbp_ci(m_3x3, 2)
        ci_r4 = self.mrelbp_ci(m_3x3, 4)
        ci_r6 = self.mrelbp_ci(m_3x3, 6)
        write_to_filecheck("check_ci/ci_r2_py.txt", ci_r2)
        write_to_filecheck("check_ci/ci_r4_py.txt", ci_r4)
        write_to_filecheck("check_ci/ci_r6_py.txt", ci_r6)

        NI_r2, RD_r2= self.NI_RD_descriptor(image, m_3x3, 2)
        write_to_filecheck("check_nird/ni_r2_py.txt", NI_r2)
        write_to_filecheck("check_nird/rd_r2_py.txt", RD_r2)

        NI_r4, RD_r4= self.NI_RD_descriptor(m_3x3, m_5x5, 4)
        write_to_filecheck("check_nird/ni_r4_py.txt", NI_r4)
        write_to_filecheck("check_nird/rd_r4_py.txt", RD_r4)
        NI_r6, RD_r6= self.NI_RD_descriptor(m_5x5, m_7x7, 6)
        write_to_filecheck("check_nird/ni_r6_py.txt", NI_r6)
        write_to_filecheck("check_nird/rd_r6_py.txt", RD_r6)
        ...
        # for i, value in enumerate(hist_r2):
        #     print(f"Index {i}: {value}")

        # with open("histogram_py.txt", "w") as file:
    ...
    def getNI_RD(self, image_r1, image_r2, r2):
        ...
        for i in range(r2,height - r2):
            for j in range(r2,width -r2):
                ...
                # with open("check_nird/r1_desc.txt", "a") as file:
                #     file.write(r1_descriptor_str + "\n")
                # r2_descriptor_str = str(r2_descriptor)
                # with open("check_nird/r2_desc.txt", "a") as file:
...
if __name__ == "__main__":
    lbp = MRELBP()
    IMG_SIZE = 128  # Change if matrix_input.txt has other sizes
    hex_input = False  # Set True if input is in HEX

    # Load original matrix
    matrix = load_matrix("matrix_input.txt", IMG_SIZE, hex_mode=hex_input) 
    # print(matrix)
    hist = lbp.MRELBP(matrix)
    with open("histogram_py.txt", "w") as file:
        for value in hist:
            file.write(f'{value}\n')  
