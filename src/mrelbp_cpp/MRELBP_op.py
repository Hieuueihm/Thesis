...
class MRELBP():
    def __init__(self, r=[2, 4, 6, 8], p = 8, w_c = 3, w_r = [3, 5, 7, 9]):
        ...
        #   Histogram
    def fast_median(self, image, ksize):
        return cv2.medianBlur(image.astype(np.uint8), ksize)

    def median_processing(self, image):
        m_3x3 = self.fast_median(image, self.w_r[0])
        m_5x5 = self.fast_median(image, self.w_r[1])
        m_7x7 = self.fast_median(image, self.w_r[2])
    ...
    def mrelbp_ci(self, image, in_r):
        ...
        padded = cv2.copyMakeBorder(image, in_r, in_r, in_r, in_r, cv2.BORDER_CONSTANT, value=0)
        h, w = image.shape
        patches = np.lib.stride_tricks.sliding_window_view(padded, (window_size, window_size))
        sum_o = patches.sum(axis=(-1, -2))
        sum_o = sum_o[in_r:h-in_r, in_r:w-in_r]
        pixel_central = image[in_r:h-in_r, in_r:w-in_r]
        scale_value = pixel_central * kernel_area
        return (scale_value >= sum_o).astype(np.uint8)
    ...
    def jointHistogram(self, ci, ni, rd):
        index = ci * 100 + ni * 10 + rd
        flat_index = index.flatten()
        hist = np.bincount(flat_index, minlength=200)
        return hist



    def getNIDescriptor(self,NI, r, sum_patch):
        threshold = (2 * r + 1) ** 2
        descriptor = 0
        for i in range(1, 9):
            if NI[i] * threshold >= sum_patch:
                descriptor |= 1 << (i - 1)
        return descriptor
